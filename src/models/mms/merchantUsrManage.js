import * as service from '../../services/mms/merchantUsr';
...
// 基础配置信息
const namespace = 'merchantUsrManage';
const objectId = 'usrId';
const enterPath = '/mms/memberUserManage/merchantUsrManage';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'querySysList', payload: { } });
        }
      });
    },
  },
  effects: {
    ...
    * querySysList({ payload }, { call, put }) {
      ...
      const res = yield call(commonService.querySysList);
      const result = parseResponse(res);
      if (result.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { sysInfoList: result.rspData },
        });
      }
    },
