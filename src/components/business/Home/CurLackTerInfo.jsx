...
class CurLackTerInfo extends React.Component {
  componentDidMount() {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.info.length !== this.props.info.length) {
      this.buildChart(nextProps.info);
  ...
  buildChart(data, count) {
    const option = {
      title: {
        text: count === 0 ? bmap.noLackDesc : bmap.lackDesc.replace('x', count),
        left: 'left',
        textStyle: {
    ...
    const chart = Echarts.init(document.getElementById('lack_ter_map'));
    chart.setOption(option);
  }

  render() {
    const terCardProps = {
      title: bmap.lackTerTitle,
      style: {
        width: '100%',
        marginBottom: 16,
      },
    ...
    return (
      <Card {...terCardProps}>
        <div id="lack_ter_map" style={{ width: '100%', height: 640 }} />
      </Card>
    );
  }
}

CurLackTerInfo.propTypes = {
  info: PropTypes.array,
  count: PropTypes.number,
};

CurLackTerInfo.defaultProps = {
  info: [
